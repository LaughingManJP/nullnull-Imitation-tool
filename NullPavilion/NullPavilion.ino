...
static void buildPavilion() {
  ...
  for (const Cube& cb : LAYOUT) {
    ...
    for (int m = 1; m <= MAXM; ++m) {
      P.S[m] = (float*)ps_malloc(P.L * sizeof(float));
...
void setup() {
  ...
  env      = (uint16_t*)heap_caps_malloc(ENVW * ENVH * 2, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (!env) env = (uint16_t*)ps_malloc(ENVW * ENVH * 2);   // 内部メモリが足りなければPSRAMへ
  noiseT   = (float*)heap_caps_malloc(NOISE * NOISE * sizeof(float), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  strip[0] = (uint16_t*)heap_caps_malloc(S * 2 * 2, MALLOC_CAP_DMA);
  strip[1] = (uint16_t*)heap_caps_malloc(S * 2 * 2, MALLOC_CAP_DMA);
  uvBuf[0] = (UV*)ps_malloc(GN * sizeof(UV));
  uvBuf[1] = (UV*)ps_malloc(GN * sizeof(UV));
  hField   = (float*)ps_malloc(GN * sizeof(float));
  baseH    = (float*)ps_malloc(GN * sizeof(float));
  pid      = (int8_t*)ps_malloc(GN);
  cls      = (uint8_t*)heap_caps_malloc(GN, MALLOC_CAP_8BIT);
  ledR     = (uint8_t*)ps_malloc(GN);

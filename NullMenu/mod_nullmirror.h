// NullMirror  -  NullMenu 用モジュール(元のスケッチを名前空間に入れただけ)
#pragma once

namespace NullMirror {
...
static uint16_t __attribute__((noinline)) MOD_IRAM crackPixel(int qi, int id) {
...
static void __attribute__((noinline)) MOD_IRAM renderBandHD(int k, uint16_t* buf, Scratch& sc)    { renderBandT<1>(k, buf, sc); }
static void __attribute__((noinline)) MOD_IRAM renderBandLight(int k, uint16_t* buf, Scratch& sc) { renderBandT<2>(k, buf, sc); }
...
bool begin() {
  ...
  noiseH = (float*)memAlloc(NOISE * NOISE * sizeof(float));
  for (int i = 0; i < NSLOT; ++i) slotBuf[i] = (uint16_t*)memCaps(S * BAND * 2, MALLOC_CAP_DMA);
  pmap   = (uint8_t*)memPs(S * S);
  crk    = (int8_t*)memCaps(CRKN * CRKN * 2, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (!crk) crk = (int8_t*)memPs(CRKN * CRKN * 2);
  ...
  for (int c = 0; c < 2; ++c) {
    scr[c].rx = (int32_t*)memCaps(CW * sizeof(int32_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    scr[c].ry = (int32_t*)memCaps(CW * sizeof(int32_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  }
  ...
  if (heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT) > envBytes + 48 * 1024)
    env = (uint16_t*)memCaps(envBytes, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  envFast = (env != nullptr);
  if (!env) env = (uint16_t*)memPs(envBytes);
  if (!env) { memFreeAll(); return false; }

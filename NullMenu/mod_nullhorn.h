// NullHorn  -  NullMenu 用モジュール(元のスケッチを名前空間に入れただけ)
#pragma once
...
bool begin() {
  ...
  lut     = (uint16_t*)memAlloc(LUTN * sizeof(uint16_t));
  rtabQ4  = (uint16_t*)memAlloc(LUTN * sizeof(uint16_t));
  ringCol = (uint16_t*)memAlloc(NR * sizeof(uint16_t));
  phTab   = (uint32_t*)memAlloc(NR * sizeof(uint32_t));
  ampC    = (float*)memAlloc(NR * sizeof(float)); ampH = (float*)memAlloc(NR * sizeof(float));
  addR    = (float*)memAlloc(NR * sizeof(float)); addG = (float*)memAlloc(NR * sizeof(float));
  addB    = (float*)memAlloc(NR * sizeof(float));
